...
import statsBanner from '@/uiControllers/statsBanner.json'
...
const NumberSpringBanner: React.FC = () => {
  ...
  //     icon: null,
  //   },
  //   { name: 'Awards', description: '2013 - Present', number: '50',year: '2021 -2022', icon: null },
  // ];
      ...
      <div className="w-full xl:w-[1140px] 2xl:w-[1440px] grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-x-8 gap-y-12 text-white text-xl py-8 z-10">
        {statsBanner.map((product, index) => (
          <div key={index} className="flex flex-col justify-center items-center w-full">
            ...
            <div className="uppercase text-center text-sm">{product.description}</div>
          </div>
        ))}
      </div>
    </div>
